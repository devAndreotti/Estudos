...
#
    ...
    def identificaDedosLevantados(self, listRef):
        dedosLevantados = [0, 0, 0, 0, 0]
        for id in range(0, 4):
           if listRef[self.prDedosmao[id]][2] < listRef[self.prDedosmao[id]-2][2]:
            dedosLevantados[id+1]=1
        
        # VRF dedão à esquerda ou direita do dedo mindinho
        if listRef[2][1] < listRef[17][1]:
            #
            # Testa dedao "abaixado"
            #
            # print("Dedao a esquerda")
            if listRef[4][1] < listRef[2][1]:
                dedosLevantados[0]=1
        elif listRef[4][1] > listRef[2][1]:
            dedosLevantados[0]=1
        #print(dedosLevantados)
